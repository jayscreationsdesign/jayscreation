...
async function importProducts() {
  ...
  try {
    ...
    // Mappe vers la structure Supabase avec IDs uniques
    const rows = products.map(p => ({
      id: p.id, // Utiliser l'ID existant
      nom: p.name || '',
      slug: p.slug || '',
    ...
    // D'abord supprimer les produits existants pour éviter les conflits
    console.log('Suppression des produits existants...')
    const { error: deleteError } = await supabase
      .from('products')
      .delete()
      .neq('id', '00000000-0000-0000-0000-000000000000') // Supprimer tout sauf un ID inexistant
