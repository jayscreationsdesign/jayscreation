...
// Test de connexion Supabase avec variables chargées
async function testSupabaseWithEnv() {
  console.log('???? Test de connexion Supabase avec variables .env.local...\n');
  ...
  try {
    ...
    // Test simple : essayer de récupérer la session
...
testSupabaseWithEnv()
  .then(result => {
    if (result.success) {
      ...
      console.log('\n???? La connexion Supabase fonctionne correctement en local.');
      console.log('???? Le problème sur Vercel vient probablement des variables d\'environnement.');
    } else {
      console.log('\n???? Test échoué !');
